...
export async function POST(req: Request, context: { params: Promise<{ broadcastId: string }> }) {
  const { broadcastId } = await context.params;
  const body = (await req.json().catch(() => ({}))) as { reason?: string };

  const backendResponse = await fetch(`${backendBaseUrl}/live-broadcasts/admin/${broadcastId}/end/`, {
    method: "POST",
