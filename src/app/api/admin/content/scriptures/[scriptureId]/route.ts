...
export async function POST(req: Request, context: { params: Promise<{ scriptureId: string }> }) {
  ...
  const destination = new URL("/scripture-of-the-day", req.url);
  if (backendResponse.ok) {
    destination.searchParams.set("saved", "1");
  } else {
    const message = await extractBackendErrorMessage(backendResponse, "Unable to update this scripture. Please try again.");
    destination.searchParams.set("edit", scriptureId);
