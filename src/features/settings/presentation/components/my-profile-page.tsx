...
function SettingsCard({
  ...
  return (
    <section
      className={`rounded-[14px] bg-[var(--color-surface-elevated)] px-4 py-4 shadow-[0_0_0_1px_rgba(255,255,255,0.05)] md:px-5 ${
...
function ProfilePicture({ viewModel }: { viewModel: MyProfileViewModel }) {
  return (
    <div className="relative mb-5">
      <div className="flex h-[112px] w-[112px] items-center justify-center overflow-hidden rounded-full bg-[var(--color-text-secondary)]">
        {viewModel.hasProfileImage ? (
          <div className="h-full w-full bg-[radial-gradient(circle_at_50%_35%,#684b44_0,#58413b_26%,var(--color-text-secondary)_27%,var(--color-text-secondary)_40%,#0000_41%),linear-gradient(180deg,#6b655f_0,#9a7a63_45%,#222_100%)]" />
        ) : (
          <svg viewBox="0 0 24 24" aria-hidden="true" className="h-12 w-12 text-[var(--color-surface-muted)]" fill="none">
      ...
      {viewModel.showPictureMenu ? (
        <div className="absolute left-0 top-[92px] z-10 w-[100px] overflow-hidden rounded-[6px] border border-white/8 bg-[var(--color-surface-muted)] text-[9px] shadow-[0_14px_24px_rgba(0,0,0,0.4)]">
...
function PasswordModal({ screen }: { screen: MyProfileViewModel["screen"] }) {
  return (
    <div className="fixed inset-0 z-30 bg-black/55 px-4 py-10">
      <div className="mx-auto max-w-[384px] overflow-hidden rounded-[14px] bg-[var(--color-surface-elevated)] shadow-[0_20px_40px_rgba(0,0,0,0.45)]">
        ...
        <div className="space-y-4 px-4 py-4">
          <label className="block">
            ...
            <input value="" readOnly aria-label="Current Password" placeholder="Enter Current Password" className="h-10 w-full rounded-[6px] border border-white/6 bg-[var(--color-surface-muted)] px-3 text-[12px] text-white/70 outline-none" />
          </label>
          <label className="block">
            <span className="mb-2 block text-[12px] text-white">New Password</span>
            <input value="" readOnly aria-label="New Password" placeholder="Enter New Password" className="h-10 w-full rounded-[6px] border border-white/6 bg-[var(--color-surface-muted)] px-3 text-[12px] text-white/70 outline-none" />
          </label>
          <label className="block">
            <span className="mb-2 block text-[12px] text-white">Confirm Password</span>
            <input value="" readOnly aria-label="Confirm Password" placeholder="Confirm New Password" className="h-10 w-full rounded-[6px] border border-white/6 bg-[var(--color-surface-muted)] px-3 text-[12px] text-white/70 outline-none" />
          </label>
        ...
        <div className="bg-[var(--color-surface-strong)] px-4 py-6">
          ...
          {viewModel.phaseState === "error" ? (
            <div className="rounded-[14px] bg-[var(--color-surface-elevated)]">
              <AdminErrorState title="Unable to load your profile" message={viewModel.errorMessage} />
            </div>
          ) : null}
