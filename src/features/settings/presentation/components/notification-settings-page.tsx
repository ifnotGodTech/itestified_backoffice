    ...
    <AdminDashboardShell viewModel={viewModel.shell}>
      <div className="max-w-[1248px] pt-4">
        ...
        <div className="bg-[var(--color-surface-strong)] px-4 py-6">
          {viewModel.phaseState === "loading" ? <div className="rounded-[14px] bg-[var(--color-surface-elevated)] px-6 py-16 text-center text-white/60">Loading notification settings...</div> : null}
          {viewModel.phaseState === "error" ? (
            <div className="rounded-[14px] bg-[var(--color-surface-elevated)]">
              <AdminErrorState title="Unable to load notification settings" message={viewModel.errorMessage} />
            </div>
          ) : null}
                ...
                {viewModel.preferences.map((preference, index) => (
                  <div key={preference.title} className="flex items-start justify-between gap-6 rounded-[12px] border border-white/6 bg-[var(--color-surface-elevated)] px-4 py-4">
                    ...
                    <label className="relative inline-flex h-6 w-12 cursor-pointer items-center">
                      ...
                      />
                      <span className="absolute inset-0 rounded-full bg-[var(--color-surface-muted)] transition-colors peer-checked:bg-[#9B68D5]" />
