...
function DetailModal({ viewModel, row, onClose }: { viewModel: ScriptureOfTheDayViewModel; row: ScriptureRow; onClose?: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-4 sm:px-6 sm:py-8">
      ...
      <div className="relative z-10 flex max-h-[calc(100vh-2rem)] w-full max-w-[760px] flex-col overflow-hidden rounded-[22px] bg-[var(--color-surface-elevated)] shadow-[0_20px_60px_rgba(0,0,0,0.35)] sm:max-h-[calc(100vh-4rem)]">
...
function EditModal({ viewModel }: { viewModel: ScriptureOfTheDayViewModel }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-4 sm:px-6 sm:py-8">
      ...
      <form action={selectedId ? `/api/admin/content/scriptures/${selectedId}` : "/api/admin/content/scriptures"} method="POST" className="relative z-10 flex max-h-[calc(100vh-2rem)] w-full max-w-[760px] flex-col overflow-hidden rounded-[22px] bg-[var(--color-surface-elevated)] shadow-[0_20px_60px_rgba(0,0,0,0.35)] sm:max-h-[calc(100vh-4rem)]">
        ...
        <div className="space-y-6 overflow-y-auto px-6 py-6">
          ...
          <label className="block space-y-2">
            ...
            <textarea name="prayer" defaultValue={draft.prayer} rows={2} className="min-h-[58px] w-full resize-none rounded-[10px] bg-[var(--color-surface-muted)] px-4 py-3 text-[15px] leading-6 text-white outline-none" />
          </label>
          <label className="block space-y-2">
            <span className="text-[15px] font-semibold text-white">Bible Text</span>
            <input name="bible_text" defaultValue={draft.bibleText} className="h-[40px] w-full rounded-[10px] bg-[var(--color-surface-muted)] px-4 text-[15px] text-white outline-none" />
          </label>
          <label className="block space-y-2">
            <span className="text-[15px] font-semibold text-white">Bible Version</span>
            <div className="relative">
            <select name="bible_version" defaultValue={draft.bibleVersion} className="h-[40px] w-full appearance-none rounded-[10px] bg-[var(--color-surface-muted)] px-4 pr-10 text-[15px] text-white outline-none">
        ...
        <div className="flex justify-end gap-4 border-t border-white/10 bg-[var(--color-surface-elevated)] px-6 pb-6 pt-4">
...
function FilterModal({ viewModel, onClose }: { viewModel: ScriptureOfTheDayViewModel; onClose?: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
      ...
      <form action="/scripture-of-the-day" className="relative z-10 w-full max-w-[380px] overflow-hidden rounded-[20px] border border-white/10 bg-[var(--color-surface-elevated)] shadow-[0_20px_60px_rgba(0,0,0,0.55)]">
        ...
        <div className="border-b border-white/10 px-5 py-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <label className="space-y-2">
              ...
              <div className="relative">
                <input name="from" defaultValue={viewModel.filterDraft.from} placeholder="dd/mm/yyyy" className="h-[40px] w-full rounded-[8px] bg-[var(--color-surface-muted)] px-10 pr-4 text-[14px] text-white/75 outline-none placeholder:text-white/30" />
            ...
            <label className="space-y-2">
              ...
              <div className="relative">
                <input name="to" defaultValue={viewModel.filterDraft.to} placeholder="dd/mm/yyyy" className="h-[40px] w-full rounded-[8px] bg-[var(--color-surface-muted)] px-10 pr-4 text-[14px] text-white/75 outline-none placeholder:text-white/30" />
                <span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-white/45">🗓</span>
              </div>
            </label>
          </div>
